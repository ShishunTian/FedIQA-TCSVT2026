...
import torch
import os
from flcore.trainmodel.TReS.posencode import PositionEmbeddingSine
from flcore.trainmodel.TReS.resnet_modify import resnet50 as resnet_modifyresnet
# import data_loader
from flcore.trainmodel.TReS.transformers import Transformer
import numpy as np
...
class TReS_net(nn.Module):
	def __init__(self, device):
		super(TReS_net, self).__init__()
...
class TReS_ori(nn.Module):
	def __init__(self, device):
		super(TReS_ori, self).__init__()
...
if __name__ == '__main__':
	...
	img = torch.randn(16, 3, 224, 224).to(device)
	model = TReS_net(device).to(device)
	print(model)
