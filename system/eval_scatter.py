...
# 加载第一个模型路径
model_path_1 = "/data/zyh26/PFL_result/Resnet/clive_FedIQA_2e-05_5e-04_101010/run_9/client_0_clive.pt"  # 替换为模型路径
# 加载第二个模型路径
model_path_2 = "/data/zyh26/PFL_result/Resnet/pfl_5_FedIQA_uniformEpoch_resnet_2e-05_5e-04_112809/run_9/client_3_clive.pt"  # 替换为模型路径
...
def predict(model, data_loader,pfl_flag):
    model.eval()
    predictions = []
    gt_scores = []
    pred_scores = []
    ...
    with torch.no_grad():
        for data, target in tqdm(data_loader, desc="Predicting", unit="batch"):
            ...
            if (pfl_flag):
                ...
                target = torch.as_tensor(remap_mos(target.to(device))).requires_grad_(False)
...
def read_IQA_data(dataset, idx, seed=2021, is_train=True, selected_data_list=[], patch_size=(224, 224), patch_num=50):
    ...
    test_index = total_num_images[int(round(0.8 * len(total_num_images))):len(total_num_images)]  # 20%测试集
...
def plot_scatter(predictions_1, predictions_2, ground_truth, model_name_1, model_name_2):
    # 设置刻度间隔
    tick_interval = 20  # 设置刻度间隔为1

    # 绘制模型1的散点图
    plt.figure(figsize=(6, 6))
    plt.scatter(ground_truth, predictions_1, color='blue', alpha=0.5)
    plt.xlabel('Ground Truth', fontsize=20)  # 设置横坐标标签字体大小
    plt.ylabel('Predictions', fontsize=20)   # 设置纵坐标标签字体大小
    plt.tick_params(axis='both', labelsize=18)  # 调整刻度字体大小
    plt.plot([min(ground_truth), max(ground_truth)], [min(ground_truth), max(ground_truth)], 'r--')

    # 自动设置刻度（只修改间隔，不限制范围）
    x_ticks = np.arange(plt.gca().get_xlim()[0], plt.gca().get_xlim()[1] + tick_interval+40, tick_interval)
    y_ticks = np.arange(plt.gca().get_ylim()[0], plt.gca().get_ylim()[1] + tick_interval+40, tick_interval)
    plt.xticks(x_ticks.astype(int))  # 横坐标刻度取整
    plt.yticks(y_ticks.astype(int))  # 纵坐标刻度取整
    ...
    # plt.xlim(0,100)
    # plt.ylim(0,100)
    x_ticks = np.arange(plt.gca().get_xlim()[0], plt.gca().get_xlim()[1] + tick_interval, tick_interval)
    y_ticks = np.arange(plt.gca().get_ylim()[0], plt.gca().get_ylim()[1] + tick_interval, tick_interval)
    # x_ticks = np.arange(0, 101, tick_interval)  # 注意范围需要和xlim一致
    # y_ticks = np.arange(0, 101, tick_interval)
...
idx = 4  # 选择你需要测试的子集（如 'csiq', 'live' 等）#'live', 'csiq', 'tid2013', 'kadid10k', 'clive', 'koniq'
...
# 进行预测
predictions_1, targets_1, srcc_1, plcc_1 = predict(model_1, test_loader,True)
